...
// Accept a meal match
V4MealMatchingAPI.mealMatchingAcceptMatch(acceptMatchRequest: acceptMatchRequest) { (response, error) in
...
// Dismiss a meal match
V4MealMatchingAPI.mealMatchingDismissMatch(dismissMatchRequest: dismissMatchRequest) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
...
// Get a food entry for review
V4MealMatchingAPI.mealMatchingGetFoodEntry(id: id) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
...
// Get suggested meal matches for a date range
V4MealMatchingAPI.mealMatchingGetSuggestions(from: from, to: to) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
