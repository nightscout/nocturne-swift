...
// Delete a custom sound.
V4AlertSoundsAPI.alertCustomSoundsDeleteSound(id: id) { (response, error) in
...
// List all custom sounds for the current tenant (metadata only, no audio data).
V4AlertSoundsAPI.alertCustomSoundsGetSounds() { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
...
// Stream the raw audio bytes for a custom sound.
V4AlertSoundsAPI.alertCustomSoundsStreamSound(id: id) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
...
// Upload a custom alert sound file.
V4AlertSoundsAPI.alertCustomSoundsUploadSound(contentType: contentType, contentDisposition: contentDisposition, headers: headers, length: length, name: name, fileName: fileName) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
