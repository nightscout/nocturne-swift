...
# **pumpSnapshotGetAll**
```swift
    open class func pumpSnapshotGetAll(from: Date? = nil, to: Date? = nil, limit: Int? = nil, offset: Int? = nil, sort: String? = nil, device: String? = nil, source: String? = nil, completion: @escaping (_ data: PaginatedResponseOfPumpSnapshot?, _ error: Error?) -> Void)
```
...
// Lists records with pagination, optional date range, device, and source filtering.
PumpSnapshotAPI.pumpSnapshotGetAll(from: from, to: to, limit: limit, offset: offset, sort: sort, device: device, source: source) { (response, error) in
...
# **pumpSnapshotGetById**
```swift
    open class func pumpSnapshotGetById(id: String, completion: @escaping (_ data: PumpSnapshot?, _ error: Error?) -> Void)
```
...
// Retrieves a single record by its unique identifier.
PumpSnapshotAPI.pumpSnapshotGetById(id: id) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
