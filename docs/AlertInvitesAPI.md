...
------------- | ------------- | -------------
[**alertInvitesCreateInvite**](AlertInvitesAPI.md#alertinvitescreateinvite) | **POST** /api/v4/alert-invites | Generate an invite link for a follower to join an escalation step.
[**alertInvitesRedeemInvite**](AlertInvitesAPI.md#alertinvitesredeeminvite) | **POST** /api/v4/alert-invites/{token}/redeem | Redeem an invite token.
[**alertInvitesRevokeInvite**](AlertInvitesAPI.md#alertinvitesrevokeinvite) | **DELETE** /api/v4/alert-invites/{id} | Revoke an unredeemed invite.
[**alertInvitesValidateInvite**](AlertInvitesAPI.md#alertinvitesvalidateinvite) | **GET** /api/v4/alert-invites/{token} | Validate an invite token (public endpoint for redemption flow).
...
// Generate an invite link for a follower to join an escalation step.
AlertInvitesAPI.alertInvitesCreateInvite(createAlertInviteRequest: createAlertInviteRequest) { (response, error) in
...
// Redeem an invite token.
AlertInvitesAPI.alertInvitesRedeemInvite(token: token) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
...
// Revoke an unredeemed invite.
AlertInvitesAPI.alertInvitesRevokeInvite(id: id) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
...
// Validate an invite token (public endpoint for redemption flow).
AlertInvitesAPI.alertInvitesValidateInvite(token: token) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
